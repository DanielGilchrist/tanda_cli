  ...
  module API
    class Client
      struct Request
        ...
        def get(type : T.class, endpoint : String, query : TQuery? = nil) : API::Result(T) forall T
          response = exec(GET, endpoint, query: query)
          API::Result(T).from(response)
        end

        def get(type : T.class, endpoint : String, query : TQuery? = nil, &block : T -> T | E) : API::Result(T) forall T, E
          response = exec(GET, endpoint, query: query)
          API::Result(T).from(response, &block)
        end

        def post(type : T.class, endpoint : String, body : TBody) : API::Result(T) forall T
          response = exec(POST, endpoint, body: body)
          API::Result(T).from(response)
        end
