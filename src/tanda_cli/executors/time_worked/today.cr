      ...
      class Today < Base
        def execute
          ...
          if total_time_worked.zero? && total_leave_hours.zero?
            @context.io.puts "You haven't clocked in today"
          end

          unless total_time_worked.zero?
            @context.io.puts("You've worked #{total_time_worked.total_hours.to_i} hours and #{total_time_worked.minutes} minutes today")
          end

          return if total_leave_hours.zero?

          @context.io.puts("You took #{total_leave_hours.hours} hours and #{total_leave_hours.minutes} minutes of leave today")
        end
      end
    end
  end
